...
q1 <- ggplot(data = data_wk_sanjuan) +
  geom_point(mapping = aes(x = Week, y = Cases), color = "red")+ggtitle('Sanjuan')

q2 <- ggplot(data = data_wk_sanjuan) +
  geom_point(mapping = aes(x = Week, y = Rain), color = "blue")

q3 <- ggplot(data = data_wk_iquitos) +
  geom_point(mapping = aes(x = Week, y = Cases), color = "red")+ggtitle('Iquitos')

q4 <- ggplot(data = data_wk_iquitos) +
  geom_point(mapping = aes(x = Week, y = Rain), color = "blue")

q5 <- ggplot(data = data_wk_ahmedabad) +
  geom_point(mapping = aes(x = Week, y = Cases), color = "red")+ggtitle('Ahmedabad')

q6 <- ggplot(data = data_wk_ahmedabad) +
  geom_point(mapping = aes(x = Week, y = Rainfall), color = "blue")
...
figure1 %<>%
  fill_panel(q2, column = 1, row = 1) %<>%
  fill_panel(q1, column = 1, row = 2) %<>%
  fill_panel(q4, column = 2, row = 1) %<>%
  fill_panel(q3, column = 2, row = 2) %<>%
  fill_panel(q6, column = 3, row = 1) %<>%
  fill_panel(q5, column = 3, row = 2) 
figure1
